...
sub extractMGs {
	...
	if ( -e $outdir ) {
	    die "\nThe output directory name \"$outdir\" already exists! Please choose a different output directory name.\n\n";
	    #system "rm -r $outdir;";
	    #mkdir $outdir;
	}
	else {
...
sub printSequences {
	...
	foreach my $strCurrentCOG (@arrCOGsIDs) {
	    ...
	    if ($strCurrentCOG ne "COG0086"){
		my $cmd = $bin . "seqtk subseq -t $strProteinFastaFileName $strIDFilesOutdir/$strCurrentCOG.IDs.txt | perl -lane 'print \">\$F[0]\\n\$F[-1]\"' > ${outdir}\/${strCurrentCOG}.faa";
		system $cmd;

		if ( $protein_only  == 0 ) {
		    $cmd = $bin . "seqtk subseq -t $strDNAFastaFileName $strIDFilesOutdir/$strCurrentCOG.IDs.txt | perl -lane 'print \">\$F[0]\\n\$F[-1]\"' > ${outdir}\/${strCurrentCOG}.fna";
		    system $cmd;
		}
	    }
	}
...
Path to executables used by this script (hmmsearch, seqtk). 
default = "$pathInWhichThisScriptResides/bin" 
If set to "" will search for executables in \$PATH
...
Path to bitscore cutoff file; Path to bitscore cutoff file; 
default = "$pathInWhichThisScriptResides/lib/MG_BitScoreCutoffs.uncalibrated.txt" (depending on -v option)
...
The B<fetchMGs> package was developed by Shinichi Sunagawa and Daniel R Mende. External software used by the B<fetchMGs> package are copyright respective authors. 

=head1 Copyright

Copyright (c) 2019 Shinichi Sunagawa, Daniel R Mende. fetchMGs is released under the GNU General Public Licence v3 (http://www.gnu.org/licenses/gpl.html). 
